...
blockForm[1].addEventListener("submit", (event) => {
  ...
  if (
    !CheckInput(controlReg) ||
    (radioESL.checked == false && radioCareer.checked == false)
  ) {
    event.preventDefault();
...
function radioChange(e) {
    ...
    let options = "";
    if (e.currentTarget.getAttribute("id")==="ESL") {
      ESLMap.forEach((program, key) => {
    ...
    courseSelected.appendChild(select);
  }
inputBox[10].addEventListener("change", radioChange);
inputBox[11].addEventListener("change", radioChange);
